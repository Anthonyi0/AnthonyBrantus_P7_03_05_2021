<!--Création de la page HTML user (boostrap)--> 
<template>
  <main class="main">
    <div class="row p-2">
      <div class="col-12">
        <h1>
          <i class="fas fa-cog"></i>Paramètres du compte
        </h1>
        <hr />
      </div>
    </div>
    <section class="row user-info pl-5 pr-5">
      <div class="col-12">
        <h2>
          <u>Informations de l'utilisateur</u>
        </h2>
        <div class="user-info__block">
          <p class="user-info__block__title mb-0">Email</p>
          <p class="user-info__block__output">
            <small>{{user.email}}</small>
          </p>
        </div>
        <div class="user-info__block">
          <p class="user-info__block__title mb-0">Username</p>
          <p class="user-info__block__output">
            <small>{{user.username}}</small>
          </p>
        </div>
        <div class="user-info__block d-sm-flex justify-content-between user-info__block--flex">
          <div class="user-info__block">
            <p class="user-info__block__title mb-0">Password</p>
            <small class="user-info__block__output">
              Doit contenir au minimum 8 caractères dont une majuscule,
              et au minimum un caractère numérique et un caractère spécial
            </small>
          </div>
          <button
            type="button"
            class="btn btn-primary"
            data-toggle="modal"
            data-target="#BoxModalChangePassword"
            @click="testInputs"
          >Change</button>
          <!--Box Modal pour changement password-->
          <div
            class="modal fade"
            id="BoxModalChangePassword"
            tabindex="-1"
            role="dialog"
            aria-labelledby="BoxModalChangePassword__title"
            aria-hidden="true"
          >
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="BoxModalChangePassword__title">Change Password</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
                <div class="modal-body">
                  <form>
                    <div class="form-group">
                      <label for="InputNewPassword">Entrez un nouveau password</label>
                      <input
                        type="password"
                        class="form-control"
                        id="InputNewPassword"
                        v-model="changePassword.newPassword"
                      />
                      <small
                        id="emailHelp"
                        class="form-text text-muted"
                      >Au minimum 8 caractères dont une majuscule, un minuscule, un caractère numérique et un caractère spécial</small>
                    </div>
                    <div class="form-group">
                      <label for="RepeatInputNewPassword">Répétez votre nouveau password</label>
                      <input
                        type="password"
                        class="form-control"
                        id="RepeatInputNewPassword"
                        v-model="changePassword.RepeatNewPassword"
                      />
                    </div>
                  </form>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                  <button type="button" class="btn btn-primary" @click="changepassword">Save changes</button>
                </div>
                <p id="retour-api" class="text-center">{{retourAPI}}</p>
              </div>
            </div>
          </div>
          <!--END: Box Modal pour changement Password-->
        </div>
        <button
          type="button"
          class="btn btn-danger white d-block mx-auto mt-5 mb-2"
          @click="deleteAccount"
        >Delete account</button>
      </div>
    </section>
  </main>
</template>

<!--Scrpit à utiliser pour user-->
<script>
//importation
import axios from "axios";
import { mapState } from "vuex";
export default {
  name: "User", //nom 
  data() { //fonction 
    return { //retourne la réponse de API et du changement password 
      retourAPI: "",
      changePassword: {
        newPassword: null,
        RepeatNewPassword: null
      }
    };
  },
  computed: {
    ...mapState(["user"]) //ont utilsise le state de user 
  },
  methods: { //ont crée les méthodes
    deleteAccount() { //fonction pour supprimée le compte 
      axios //utilisation axios 
        .delete("http://localhost:3000/api/user/delete", { //utilisiation de delete à l'adresse 
          headers: {//ont impose un header 
            Authorization: "Bearer " + localStorage.getItem("token") //ont à besoin de l'autorisation token dans le localstorage 
          }
        })
        .then(() => { //on attend un promise
          alert('Votre compte et vos messages ont bien été supprimé') 
          localStorage.clear(); //ont clear le localstorage 
          location.replace(location.origin+'/#/signup'); //et on envoie l'utilisateur à la création du compte 
          document.location.reload();
        })
        .catch(error => console.log(error)); //ont catche en cas d'erreur
    },
    changepassword() { //fonction changement mots de passe 
      //Controle de la saisie du nouveau password
      //Controle de repeat et non null
      if (
        this.changePassword.newPassword == this.changePassword.RepeatNewPassword &&
        this.changePassword.newPassword != "" &&
        this.changePassword.RepeatNewPassword != ""
      ) {
        axios //utilisation axios
          .put("http://localhost:3000/api/user/update",{ //ont utilise put sur l'adresse
              newPassword: this.changePassword.newPassword 
            },
            {
              headers: {//utilisation de header
                Authorization: "Bearer " + localStorage.getItem("token") //besoin de l'autorisation token 
              }
            }
          )
          .then(response => { //promise avec réponse 
            console.log("password change", response); //on log 
            document.getElementById("retour-api").classList.add("text-success");
            this.retourAPI = response.data.confirmation;
            setTimeout(() => {
              this.retourAPI = "";
              alert('Votre mots de passe à bien été changez')
              window.location.reload();
            },);
          })
          .catch(error => { //catch en cas d'erreur 
            console.log("admin", error);
            this.retourAPI = "Une erreur est survenue, vérifier vos saisies";
          });
      } else {
        document.getElementById("retour-api").classList.add("text-danger");
        this.retourAPI = "Veuillez vérifier la saisie des mots de passe";
      }
    },
    testInputs() { //vérification des inputs 
      //8 caractères dont au minimum une majuscule, une minuscule, un caractère numérique et un caractère spécial
      const regexPassword = /((?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[\W]).{8,64})/;
      let inputNewPassword = document.getElementById("InputNewPassword");
      let inputRepeatNewPassword = document.getElementById("RepeatInputNewPassword");
      inputNewPassword.addEventListener("input", function(e) {
        let value = e.target.value;
        let testValue = regexPassword.test(value);
        if (testValue) {
          inputNewPassword.style.backgroundColor = "#4CAF50";
        } else {
          inputNewPassword.style.backgroundColor = "#f44336";
        }
      });
      inputRepeatNewPassword.addEventListener("input", function() {
        if (
          inputRepeatNewPassword.value == inputNewPassword.value &&
          regexPassword.test(inputRepeatNewPassword.value)
        ) {
          inputRepeatNewPassword.style.backgroundColor = "#4CAF50";
        } else {
          inputRepeatNewPassword.style.backgroundColor = "#f44336";
        }
      });
    }
  },
  mounted() {
    this.$store.dispatch("getUserInfos");
  }
};
</script>

<style lang="scss">
</style>